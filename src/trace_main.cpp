...
{
    std::vector<seal::Ciphertext> result; result.reserve(layer.output_size);
    for (int output = 0; output < layer.output_size; ++output) {
...
{
    try {
        const std::string model_path = argc > 1 ? argv[1] : "src/mnist_mlp_gelu.json";
        ...
        for (std::size_t index = 0; index < model.layers.size(); ++index) {
            ...
            encrypted_values = encrypted_layer(layer, encrypted_values, evaluator, relin_keys, galois_keys, encoder, scale);
